...
// ---- Plugin constants ----
if ( ! defined( 'SUBSALES_VERSION' ) ) define( 'SUBSALES_VERSION', '3.74.1' );
...
function subsales_review_queue_bulk_ajax() {
    ...
    wp_send_json_success( subsales_review_queue_bulk_pass( $skip ) );
}
 ...
 * a second copy of the logic drifting away from what the button does.
 *
 * @param array $skip Queue row ids already tried and stuck this run.
 * @return array processed/resolved/failed/stuck_ids/remaining
 * @since 3.29.0
 */
function subsales_review_queue_bulk_pass( $skip = array() ) {
    ...
    foreach ( $rows as $row ) {
        ...
        // The town stays as the parcel layer gave it. Google's reverse geocode is
        // not reliable about jurisdiction at a town line - measured on Jeffrey
        // Ln, where the assessor has every parcel in Southington and Google
        // returned Meriden for some houses and Berlin for others, 60 metres
        // apart. Only the ZIP, the thing point-in-polygon could not settle, is
        // taken from Google.
        //
        // When Google's town disagrees with the parcel's, that disagreement is
        // the signal this rooftop sits on a boundary. Leave it for a person -
        // deciding it automatically is what put wrong towns in the book before.
        if ( 'zip_undetermined' === $row['reason']
             && '' !== $place['city']
             && '' !== (string) $row['city']
             && 0 !== strcasecmp( trim( $place['city'] ), trim( $row['city'] ) ) ) {
            $failed[] = $label . ' - on a town line (parcel says ' . $row['city'] . ', Google says ' . $place['city'] . ')';
            $stuck[]  = $id;
            continue;
        }

        // A seller-typed row has no parcel pedigree - its street and town were
        // parsed out of free text and are often mangled - so it takes Google's.
...
function order_sync_fetch_orders_ajax() {
    ...
    $season_id = isset( $_POST['season_id'] ) ? intval( $_POST['season_id'] ) : Subsales_Database::current_season_id();
...
function subsales_serve_portal_assets() {
    ...
    if ( $req_path === $portal_base . '/service-worker.js' || 
        ...
        if ( file_exists( $file ) ) {
            ...
            header( 'Access-Control-Allow-Origin: *' );
            http_response_code(200);
            // The worker is served from the portal path but its assets live in
            // the plugin directory, and it cannot work that out from its own
            // scope. Hand it the answer.
            echo str_replace(
                '__SUBSALES_PLUGIN_BASE__',
                esc_url_raw( SUBSALES_PLUGIN_URL . 'pwa/' ),
    ...
    if ( $req_path === $portal_base || $req_path === $portal_base . '/index.html' || $req_path === $portal_base . '/' ) {
        ...
        if ( file_exists( $file ) ) {
            ...
            $settings = array(
                ...
                // The portal shell was the one config builder that left this
                // out, so the digital option stayed hidden on the only screen
                // that offers it however the setting was set.
                'digitalPaymentsEnabled' => (bool) get_option( 'subsales_digital_payments_enabled', false ),
            );
            ...
            //
            // Never cache the shell. It carries the ?v= that points at this
            // release's app.js, so a cached copy keeps handing sellers the
            // previous release's code - for five minutes here, and indefinitely
            // once a proxy or the browser holds it. The assets it references are
            // versioned and can be cached hard; this document cannot.
                        ...
                        if (confirm('Are you sure you want to remove this registration?')) {
                            try {
                                ...
                                if (response.ok) {
                                    ...
                                    const tookDriver = removed && removed.driver === 'removed';
                                    document.getElementById('action-status').innerHTML = '<span style="color: #28a745;">✓ Registration removed.' +
                                        (tookDriver ? ' Your driver is off this day too &mdash; tell your team they need a new one.' : '') + '</span>';
                                    setTimeout(() => {
                                        document.getElementById('signup-details-modal').style.display = 'none';
                                        loadUserSignups();
                                    }, tookDriver ? 4000 : 1500);
                                } else {
