...
### Changed
- **Save Order moved to the bottom right of the order form, Clear Form to the bottom left.** They were the other way round, so a thumb reaching for Save crossed the red Clear button on the way. Save is now the big button under the thumb; Clear is smaller, outlined rather than solid red, and a good gap away.
...
### Fixed
- **The &ldquo;paid by card&rdquo; message appeared twice on an order being edited** &mdash; once in the notice over the subs, and again as a greyed line half-hidden behind it. Only the notice shows now.
...
### Fixed
- **The suggested `{adminphone}` wording is now a single sentence** (`Call {adminphone} with any questions.`). Phrased as two sentences it left "Questions?" stranded when no number was set, since only the sentence holding the token gets dropped. The Text Messages tab now spells out that rule.
